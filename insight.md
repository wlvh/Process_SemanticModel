...
* **洞察模板（例子）**：某些队列 **P75 明显偏右**，而均值正常 → **容量峰值/排班**问题；**改 P75** 往往比拉均值更快改善口碑。
* **动作**：**高峰时段排班**与**自动应答分流**优化；对 `P75` 设 **SLA 门限**（如 4h）。

---

## 故事3｜《14天的拐点》：超龄在手是怎样“长出”DSAT的
...
* **洞察模板（例子）**：**Q3** 队列 `%>14d` 出清不力，**下月 DSAT% 抬升**；根因集中于 **RootPath=等待外部依赖**。
* **动作**：为超龄件设计**快车道**；每周拉链报表，规定**出清率**。
* **超过阈值分层策略**：如 14/30/45 天不同处置。

---

## 故事4｜《升级的回音壁》：T3/IcM 是否在放大客户噪声？
...
* **洞察模板（例子）**：**产品B** 的 **T3 Bugs/千创建** 值为同类 2.1×；`T1->IcM` 在 **站点S1** 明显偏高，或存**越级路由**。
* **动作**：联合研发设 **Bug 火线清零周**；对越级路由加 **拦截/回流规则**。
* 应关注**单位创建量的升级率**与**修复闭环时效**。

---

## 故事5｜《自助的 ROI》：Deflection 不只是美观曲线
...
* **洞察模板（例子）**：**产品C** 分流率高但 DSAT% 未降，说明**自助误杀**或**内容误导**；应做**内容 A/B** 与**召回率/精确率**监控。
* **动作**：为高流量问题建立**决策树/对话编排**；把低置信度意图**快速转人工**。
* **误区**：只追分流率，不看**人工回流率**与**后续满意度**。

---

## 故事6｜《语言与公平》：本地化不是翻译，是时效与路径公平
...
* **洞察模板（例子）**：**语言L2** 在 **站点S2** 的首响尾部长 + 低 FCR，建议**路由回平**或**补能**。
* **动作**：路由策略按 SLA 对不同语言设**守门阈**；多语知识库**质量门**。
* **误区**：把语言问题简单归因“沟通效率”；常见根因是**运力配置/路由**。

---

### 故事7｜《首周定生死》：%FWR 的真实商业价值
